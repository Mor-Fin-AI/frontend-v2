...
import {
  buildDaoEducationMetrics,
  daoEducationRewardsMetrics,
  type DaoEducationMetric,
} from "../data";

function MetricValue({ metric }: { metric: DaoEducationMetric }) {
...
export default function DaoEducationRewardsPanel({
  isLoading = false,
}: {
  isLoading?: boolean;
}) {
  const metrics = useMemo(
    () => buildDaoEducationMetrics(daoEducationRewardsMetrics),
    []
  );
  ...
  return (
    <DashboardStatCardsGrid
      isLoading={isLoading}
      loadingLabel="Loading DAO education rewards metrics"
    >
      {metrics.map((metric) => (
